...
- **Image-only response modality**: `client.interactions.create` is always called with `response_modalities=["IMAGE"]` (never `["TEXT", "IMAGE"]`). With TEXT allowed as a response modality, these models will sometimes return a prose *description* of the image instead of drawing it -- especially on elaborate prompts with reference images. Omitting TEXT removes that escape hatch. `aspect_ratio` in `response_format` works in both Developer API and Enterprise mode; `mime_type` is only ever sent (as `"image/jpeg"`) when the configured output MIME type is explicitly jpeg -- the SDK only supports requesting jpeg explicitly, so PNG output (the default) is left to the API's own default rather than requested.
